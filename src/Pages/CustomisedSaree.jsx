...
const CustomisedSarees = () => {
  const [products, setProducts] = useState([]);
  const [loading, setLoading] = useState(true);

  const fetchCustomisedSarees = async () => {
    try {
      const { data } = await api.get("/products/getAll");
      const filtered = data?.products?.filter(
        (item) =>
          item.category?.toLowerCase() === "saree" &&
          item.sareeType?.toLowerCase() === "customised"
      );
      setProducts(filtered || []);
    } catch (err) {
      console.error("Failed to fetch customised sarees:", err);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="px-4 md:px-8 py-12 max-w-[1600px] mx-auto">
      <div className="text-center mb-10">
        <h2 className="text-3xl md:text-4xl font-light tracking-wide">
          Customised Saree Collection
        </h2>
        <p className="text-gray-600 mt-2 max-w-xl mx-auto">
          Explore our handpicked range of customised designer sarees crafted with love.
        </p>
      </div>

      {loading ? (
        <p className="text-center text-gray-500 text-lg">Loading customised sarees...</p>
      ) : products.length === 0 ? (
        <p className="text-center text-gray-500 text-lg">No customised sarees found.</p>
      ) : (
...
export default CustomisedSarees;
