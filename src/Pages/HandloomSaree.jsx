...
const HandloomSaree = () => {
  ...
  const fetchHandloomSarees = async () => {
    try {
      const { data } = await api.get("/products/getAll");
      const filtered = data?.products?.filter(
        (item) =>
          item.category?.toLowerCase() === "saree" &&
          item.sareeType?.toLowerCase() === "handloom"
      );
      setProducts(filtered || []);
    } catch (err) {
      console.error("Failed to fetch handloom sarees:", err);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="px-4 md:px-8 py-12 max-w-[1600px] mx-auto">
      <div className="text-center mb-10">
        ...
        <p className="text-gray-600 mt-2 max-w-xl mx-auto">
          Discover our elegant range of handwoven handloom sarees made with traditional craft.
        </p>
      </div>

      {loading ? (
        <p className="text-center text-gray-500 text-lg">Loading handloom sarees...</p>
      ) : products.length === 0 ? (
        <p className="text-center text-gray-500 text-lg">No handloom sarees found.</p>
      ) : (
